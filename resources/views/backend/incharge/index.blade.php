        ...
        <div class="page-inner">
            ...
            <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
                ...
                <div class="ms-md-auto py-2 py-md-0">
                    ...
                    <a href="{{ route('incharge.customers.create') }}" class="btn btn-primary btn-round">Add Customers</a>
                </div>
            </div>
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header d-flex align-items-center justify-content-between client">
                            <div class="card-title">Recents Customers</div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header d-flex align-items-center justify-content-between client">
                            <div class="card-title">Recent Agents</div>
                            <div class="d-flex" style="gap:8px; align-items:center;">
                                <div>
                                    <label for="agents-per-page-select" class="me-2">Per Page:</label>
                                    <select id="agents-per-page-select" class="form-select form-select-sm" style="width:auto; display:inline-block;">
                                        <option value="10" selected>10</option>
                                        <option value="25">25</option>
                                    </select>
                                </div>
                                <div class="input-group input-group-sm" style="width:360px;">
                                    <input type="text" id="agents-search_items" class="form-control" placeholder="Search name, phone, address, nid...">
                                    <button type="button" id="agents-search-clear" class="btn btn-outline-secondary" title="Clear search"><i class="fas fa-times"></i></button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <div id="agents-pagination-info" class="text-muted small"></div>
                            </div>
                            <div class="table-responsive">
                                <table class="display table table-striped table-hover">
                                    <thead class="text-center">
                            ...
                            <nav aria-label="Page navigation" class="mt-3">
                                <ul class="pagination justify-content-center" id="agents-pagination-container"></ul>
                            </nav>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    <script type="text/javascript">
        ...
        // Agents AJAX pagination
        (function(){
            const tbody = document.getElementById('agents-table-body');
            const searchInput = document.getElementById('agents-search_items');
            const clearBtn = document.getElementById('agents-search-clear');
            const perPageSelect = document.getElementById('agents-per-page-select');
            const paginationContainer = document.getElementById('agents-pagination-container');
            const paginationInfo = document.getElementById('agents-pagination-info');
            if (!tbody) return;
                    ...
                    .then(json => {
                        ...
                        data.forEach((a, idx) => {
                            ...
                            tr.innerHTML = `
                                ...
                                <td class="text-center">${escapeHtml(a.address || '')}</td>
                                <td class="text-start">${escapeHtml(a.nid_number || '')}</td>
                                <td class="text-center">${escapeHtml(statusText)}</td>
                                <td class="text-center">${escapeHtml(approvedText)}</td>
                                <td class="text-center">${escapeHtml(a.balance)}</td>
                                <td class="text-center">${escapeHtml(a.created_at || '')}</td>
                                <td class="text-end">
                                    <a href="${editUrl}" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></a>
                                    <button type="button" class="btn btn-sm btn-danger" onclick="showDeleteAgentConfirm(${a.id})"><i class="fa fa-trash"></i></button>
                                </td>
                            `;
                            tbody.appendChild(tr);
                        });
                        ...
                        // Build pagination links
                        buildAgentsPagination(pagination);
                    })
                    .catch(err => { console.error('Failed to load agents', err); });
            }

            function buildAgentsPagination(pagination) {
                paginationContainer.innerHTML = '';
                const perPage = perPageSelect.value;
                if (perPage === 'all' || pagination.last_page <= 1) return;
            ...
            const debouncedFetch = debounce(() => { currentPage = 1; fetchAgents(1); }, 350);

            if (searchInput) searchInput.addEventListener('input', debouncedFetch);
            if (clearBtn) clearBtn.addEventListener('click', function(){ searchInput.value = ''; currentPage = 1; fetchAgents(1); });
    ...
    <script>
        ...
        function showDeleteAgentConfirm(id) {
            event.preventDefault();
